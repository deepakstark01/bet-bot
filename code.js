...
const requiredWinPatterns = 2; // Number of valid patterns needed
...
function stopAutoBet() {
  stopButton = document.querySelector('button[data-test="auto-bet-button"]');
  if (stopButton) {
    stopButton.click();
...
function notifyUser() {
  console.log('Required win patterns achieved. Notifying user...');
  beepboop();
}
...
async function initiateBettingCycle() {
  ...
  await delay(3000);
  await startAutoBet(); // Wait for the start of betting
  checkResults();
}
